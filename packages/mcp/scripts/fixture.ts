// Prints the plugin's envelopes for the shared interop fixture. Run with
...
const fx = JSON.parse(await readFile(new URL("../test/fixtures/v2.json", import.meta.url), "utf8")) as { plaintext: string; key: string; passphrase: string };
// Fragment mode with the fixture's fixed key rather than a random one, so the
// Go test can open it with the same key.
...
const { envelope: pbkdf2 } = await seal(fx.plaintext, fx.passphrase);
console.log(JSON.stringify({ none, pbkdf2 }, null, 2));
