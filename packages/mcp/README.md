...
| `fmrl_publish_file` | `path` to a `.html`, `.htm`, `.md`, `.markdown`, `.mdx` or `.txt` file (2 MiB at most), optional `title`, optional `private` (encrypts the page here before upload; the link carries the key after `#p=`), optional `passphrase` (implies private; readers type it on the page instead) → the same as `fmrl_publish`, one-time browser link included |
